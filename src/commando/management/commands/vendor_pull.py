...
class Command(BaseCommand):
    def handle(self, *args:Any, **options:Any):
        self.stdout.write('Starting to downloas vendor files')
        completed_urls = []
        for name, url in VENDOR_STATICFILES.items():
            file_path = STATICFILES_VENDOR_DIR / name
            dl_success = download_to_local(url, file_path)
            if dl_success:
        ...
        if set(completed_urls)==set(VENDOR_STATICFILES.values()):
            self.stdout.write('Successfully downloaded the vendor files')
        else:
            self.stdout.write(self.style.WARNING('Some file were not downloaded'))
